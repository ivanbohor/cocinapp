...
export default function AdminProducts() {
  ...
  // Ref para el input de archivo oculto (CSV)
  ...
  // ===============================================
  // LÓGICA DE SELECCIÓN MÚLTIPLE Y ELIMINACIÓN EN LOTE
  // ===============================================
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === filtered.length) {
      ...
      setSelectedIds(filtered.map(p => p.id)); // Seleccionar todos los filtrados
    }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!window.confirm(`¿Estás seguro de eliminar los ${selectedIds.length} productos seleccionados?`)) return;
  ...
  // ===============================================
  // LÓGICA DE EXPORTACIÓN A CSV (DESCARGAR TABLA)
  // ===============================================
  const handleExportCSV = () => {
    ...
    // Crear archivo descargable temporal en el navegador
  ...
  // ===============================================
  // LÓGICA DE IMPORTACIÓN MASIVA (CSV/EXCEL)
  // ===============================================
  ...
  const procesarCSV = async (csvText: string) => {
    ...
    try {
      ...
      const nuevosProductos = lines.slice(1).map(line => {
      ...
      if (nuevosProductos.length === 0) throw new Error('No se encontraron productos válidos.');

      const { data, error } = await supabase.from('productos').insert(nuevosProductos).select();
      if (error) throw error;

      if (data) setProductos(prev => [...prev, ...data]);
      alert(`¡Éxito! Se importaron ${data?.length || 0} productos correctamente.`);
      
    } catch (error: any) {
      console.error('Error importando CSV:', error);
      alert(`Error al importar: ${error.message}\nAsegúrate de que el CSV tenga este orden: Nombre, Descripción, Precio, Categoría, ImagenURL`);
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Menú y Productos</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Gestiona los platos de tu carta.</p>
        </div>
      ...
      {/* BARRA DE ACCIONES MASIVAS (Se muestra solo si hay elementos seleccionados) */}
      {selectedIds.length > 0 && (
        <div className="flex items-center justify-between bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-900 px-4 py-3 rounded-lg animate-in fade-in">
      ...
      <div className="flex items-center bg-white p-2 rounded-lg border shadow-sm max-w-md dark:bg-slate-900 border-slate-200 dark:border-slate-800">
        <Search size={20} className="text-slate-400 mx-2" />
        <Input placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="border-0 shadow-none dark:bg-slate-900 dark:text-white focus-visible:ring-0" />
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 dark:bg-slate-950/50 text-slate-700 dark:text-slate-300 border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th className="px-4 py-4 w-10 text-center">
                <button onClick={toggleSelectAll} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {filtered.map((prod) => {
              ...
              return (
                <tr key={prod.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${isSelected ? 'bg-indigo-50/50 dark:bg-indigo-950/20' : ''}`}>
                  <td className="px-4 py-4 text-center">
                    <button onClick={() => toggleSelectOne(prod.id)} className="text-slate-400 hover:text-slate-700 dark:hover:text-white">
                      {isSelected ? (
                        <CheckSquare size={18} className="text-indigo-600 dark:text-indigo-400" />
                      ) : (
                        <Square size={18} />
                  ...
                  <td className="px-6 py-4">
                    ...
                    {prod.description && <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-xs">{prod.description}</p>}
                  </td>
                  <td className="px-6 py-4">
                    <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-md text-xs font-medium border border-transparent dark:border-slate-700">
                      {prod.category}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-black dark:text-white">${prod.price.toLocaleString()}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${prod.status === 'Activo' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                      {prod.status}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-800">
            ...
            <div className="p-5 space-y-2 max-h-96 overflow-y-auto">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">Usa las flechas para elegir el orden en el que tus clientes verán las categorías en la carta web.</p>
              {categoriasActivas.map((cat, index) => (
                <div key={cat} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8 dark:text-slate-300" onClick={() => moverCategoria(index, 'arriba')} disabled={index === 0}><ArrowUp size={16} /></Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 dark:text-slate-300" onClick={() => moverCategoria(index, 'abajo')} disabled={index === categoriasActivas.length - 1}><ArrowDown size={16} /></Button>
                  </div>
                </div>
              ))}
              <Button onClick={guardarOrden} disabled={isSubmitting} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white mt-4 border-0">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-800">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
              <h3 className="font-bold text-slate-900 dark:text-white">{activeId ? 'Editar Producto' : 'Nuevo Producto'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400"><X size={20} /></button>
            </div>
            <form onSubmit={handleSave} className="p-5 space-y-4">
              <div className="space-y-1.5"><label className="text-sm font-medium dark:text-slate-300">Nombre</label><Input required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="dark:bg-slate-800 dark:text-white dark:border-slate-700" /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><label className="text-sm font-medium dark:text-slate-300">Precio ($)</label><Input required type="number" step="0.01" value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})} className="dark:bg-slate-800 dark:text-white dark:border-slate-700" /></div>
                <div className="space-y-1.5"><label className="text-sm font-medium dark:text-slate-300">Estado</label><select value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})} className="flex h-10 w-full rounded-md border border-slate-200 bg-transparent px-3 text-sm outline-none dark:bg-slate-800 dark:text-white dark:border-slate-700"><option value="Activo">Activo</option><option value="Pausado">Pausado</option></select></div>
              </div>
              <div className="space-y-1.5"><label className="text-sm font-medium dark:text-slate-300">Categoría</label><Input required value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="dark:bg-slate-800 dark:text-white dark:border-slate-700" /></div>
              <div className="space-y-1.5"><label className="text-sm font-medium dark:text-slate-300">URL Imagen (Opcional)</label><Input value={formData.image_url} onChange={(e) => setFormData({...formData, image_url: e.target.value})} className="dark:bg-slate-800 dark:text-white dark:border-slate-700" /></div>
              <Button type="submit" disabled={isSubmitting} className="w-full bg-slate-900 dark:bg-indigo-600 text-white mt-4 border-0">{isSubmitting ? <Loader2 size={18} className="animate-spin" /> : 'Guardar'}</Button>
            </form>
          </div>
        </div>
      )}
